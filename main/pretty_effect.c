...
static int8_t xofs[MAX_DISPLAY_DIMENSION], yofs[MAX_DISPLAY_DIMENSION];
static int8_t xcomp[MAX_DISPLAY_DIMENSION], ycomp[MAX_DISPLAY_DIMENSION];
...
void pretty_effect_calc_lines(uint16_t *dest, int line, int frame, int linect, int display_width, int display_height)
{
    if (frame!=prev_frame) {
        ...
        for (int x=0; x < display_width; x++) xofs[x]=sin(frame*0.15+x*0.06)*4;
        for (int y=0; y < display_height; y++) yofs[y]=sin(frame*0.1+y*0.05)*4;
        for (int x=0; x < display_width; x++) xcomp[x]=sin(frame*0.11+x*0.12)*4;
        for (int y=0; y < display_height; y++) ycomp[y]=sin(frame*0.07+y*0.15)*4;
        prev_frame=frame;
    }
